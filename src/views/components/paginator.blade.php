@if ($paginator->hasPages())
    <nav>
        <ul class="pagination mb-0">
            ...
            @else
                <li class="page-item">
                    <a class="btn btn-sm btn-outline-light text-dark border-0" wire:click.prevent="previousPage" wire:loading.class="disabled" rel="prev" aria-label="@lang('pagination.previous')"><em class="fa fa-chevron-left small text-secondary"></em></a>
                </li>
            @endif
            ...
            @foreach ($elements as $element)
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @else
                            <li class="page-item"><a class="btn btn-sm btn-outline-light text-dark border-0 shadow-none" wire:click.prevent="gotoPage({{ $page }})" wire:loading.class="disabled">{{ $page }}</a></li>
                        @endif
                    @endforeach
                @endif
            @endforeach
            ...
            @if ($paginator->hasMorePages())
                <li class="page-item">
                    <a class="btn btn-sm btn-outline-light text-dark border-0" wire:click.prevent="nextPage" wire:loading.class="disabled" rel="next" aria-label="@lang('pagination.next')">
